from database import register_user, get_categories, get_category_products, get_product, add_to_cart, get_user_id, get_cart_product_quantity, update_product_quantity, get_users_cart_amount, get_users_products_in_cart
...
async def show_cart(message: Message):
    ...
    for cart_product in products:  # ((1, 1, 1, 3, 3.60), (2, 1, 2, 3, 4.20))
        ...
        prices.append(
            LabeledPrice(label=product_name, amount=int(float(cart_product[-1]) * 100))
        )
...
async def return_shipping_options(shipping_query: ShippingQuery):
    shipping_options = [
        ShippingOption(id="fast", title="World wide teleport", prices=[LabeledPrice(label="As soon as possible", amount=10000)]),
        ShippingOption(id="normal", title="Within 30 days", prices=[LabeledPrice(label="Normal delivery", amount=2000)]),
    ]
    ...
    await bot.answer_shipping_query(
        ...
        ok=True,
        error_message="Oops, looks like we are out of these products"   
    )
...
We will proceed with your order for '{total_amount} {currency}'
Stay in touch"""
